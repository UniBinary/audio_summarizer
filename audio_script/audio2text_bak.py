...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('audio_transcription.log', encoding='utf-8'),
...
def get_file_list(input_dir: str, from_num: int, to_num: int, skip_nums: List[int]) -> List[Tuple[str, str, str]]:
    """获取要处理的文件列表"""
    files = []
    
    for num in range(from_num, to_num + 1):
        if num in skip_nums:
            logger.info(f"跳过文件: {format_number(num)}.mp3")
            continue
            
        filename = f"{format_number(num)}.mp3"
...
def main():
    ...
    # 获取文件列表
    files = get_file_list(args.input_dir, args.from_number, args.to_number, skips)
